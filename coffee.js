...
// Create a class named ThingWebCoffee and contains an initial property WoT
class ThingWebCoffee {
...
Assumes one medium americano if not specified, but time and mode are mandatory fields.`,
      ...
      .then(function (thing) {
        ...
        thing.writeProperty("possibleDrinks", [
          "espresso",
          "americano",
          "cappuccino",
          "latte",
        ...
        thing.setActionHandler("makeDrink", (params, options) => {
          ...
          return thing
            .readProperty("allAvailableResources")
            .then((resources) => {
              ...
              newResources["milk"] -= Math.ceil(
                quantity * sizeQuantifiers[size] * drinkRecipes[drinkId]["milk"]
              );
              ...
              for (let resource in newResources) {
                if (newResources[resource] <= 0) {
                  return new Promise((resolve, reject) => {
                    thing.emitEvent(
                      "outOfResource",
                      `Low level of ${resource}: ${resources[resource]}%`
                    );
                    resolve({
                      result: false,
              ...
              return thing
                .writeProperty("allAvailableResources", newResources)
                .then(() => {
        ...
        thing.setActionHandler("setSchedule", (params, options) => {
          ...
          if (
            ...
            // Use default values if not provided
            params["drinkId"] =
              "drinkId" in params ? params["drinkId"] : "americano";
            params["size"] = "size" in params ? params["size"] : "m";
            params["quantity"] = "quantity" in params ? params["quantity"] : 1;
